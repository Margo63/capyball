class Constants {
    ...
    static getMyGoal(side) {
        switch (side) {
            case 'r':
                return {name: 'gr', coords: Constants.FLAGS['gr']}
            case 'l':
                return {name: 'gl', coords: Constants.FLAGS['gl']}
            default:
                return null
        }
    }
    ...
    static getFlagInfo(fl, constant_labels) {
        const flag = {
            x: Constants.FLAGS[fl].x,
            y: Constants.FLAGS[fl].y,
            f: fl,
        }
        const index = constant_labels.findIndex(function (label) {
            return label.fl === fl
        });
        if (index !== -1) {
            flag.dist = constant_labels[index].distance
            flag.angle = constant_labels[index].angle
        }
        return flag
    }
}
